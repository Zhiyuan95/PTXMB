  ...
  const chartData = useMemo(() => {
    ...
    return days.map((day) => {
      ...
      dailyEntries.forEach(e => {
        ...
        if (selectedTemplateIds.length === 0 || selectedTemplateIds.includes(e.templateId)) {
      ...
      return {
        date: dateStr,
        displayDate: format(day, timeRange === "year" ? "MMM" : "MM-dd", { locale: zhCN }),
        amount: dailyTotal,
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      const visibleTemplates = templates.filter(t => {
         ...
         const isSelected = selectedTemplateIds.length === 0 || selectedTemplateIds.includes(t.id);
          ...
          <div className="space-y-2">
            {visibleTemplates.map(t => (
                <div key={t.id} className="flex justify-between items-center gap-4">
                    ...
                    <span className="font-bold text-[color:var(--primary)] font-display">
                        {(data[t.id] || 0).toLocaleString()} <span className="text-[10px] font-sans font-normal opacity-70">{unitLabels[t.unit]}</span>
                    </span>
                </div>
            ))}
            ...
            <div className="flex justify-between items-center gap-4 pt-2 mt-2 border-t border-[color:var(--line)]/50">
                ...
                <span className="font-bold text-[color:var(--ink)] font-display text-sm">
                    {payload[0].value.toLocaleString()} <span className="text-[10px] font-sans font-normal opacity-70">遍</span>
                </span>
            </div>
          </div>
        </div>
  ...
  return (
    <section className="glass-card p-8 rounded-[2rem] relative overflow-hidden transition-all duration-300">
      ...
      <div className="flex flex-col gap-6 mb-8 z-10 relative">
        ...
        <div className={`flex flex-wrap gap-2 transition-all duration-300 overflow-hidden ${isCollapsed ? 'max-h-0 opacity-0' : 'max-h-32 opacity-100'}`}>
            <button
      ...
      {/* Chart */}
      <div className={`h-[300px] w-full -ml-4 transition-all duration-300 overflow-hidden ${isCollapsed ? 'max-h-0 opacity-0 !mt-0' : 'max-h-[300px] opacity-100'}`}>
