...
export default function Heatmap({ entries }: HeatmapProps) {
  ...
  const availableYears = useMemo(() => {
    const years = new Set(
      entries.map((e) => getYear(parseISO(e.entryDate)))
    );
  ...
  const getColorClass = (level: number) => {
    switch (level) {
      case 1: return "bg-[color:var(--primary)]/20";
      case 2: return "bg-[color:var(--primary)]/50";
      case 3: return "bg-[color:var(--primary)]/80";
      case 4: return "bg-[color:var(--primary)]";
      default: return "bg-stone-200 dark:bg-stone-800"; // Level 0
    }
  };
        ...
        <div className="overflow-x-auto pb-4 custom-scrollbar">
            ...
            >
                ...
                {daysInYear.map((day) => {
                    ...
                    const data = intensityMap.get(dateStr) || { level: 0, count: 0 };
            ...
            <div className="relative h-6 mt-2 text-[10px] text-[color:var(--muted)] font-bold uppercase pointer-events-none">
              {availableYears.length > 0 && Array.from({ length: 12 }).map((_, i) => {
                const monthDate = new Date(selectedYear, i, 1);
                // Calculate column index
                // 1. Find index of this date in the days array (diff in days from start of year)
                const startOfCurrentYear = startOfYear(new Date(selectedYear, 0, 1));
                const dayIndex = Math.floor((monthDate.getTime() - startOfCurrentYear.getTime()) / (1000 * 60 * 60 * 24));
